"""
Test script to analyze FutureRojgar website structure.

FutureRojgar claims to have 30k+ jobs and appears to be very active.
...
def analyze_futurerojgar_structure():
    """Analyze FutureRojgar website structure."""
    print("🔍 Analyzing FutureRojgar structure...")
    
    base_url = "https://futurerojgar.com"
    
    # Test different potential job listing URLs
    test_urls = [
        "https://futurerojgar.com",
        "https://futurerojgar.com/jobs",
        "https://futurerojgar.com/job",
        "https://futurerojgar.com/vacancy",
        "https://futurerojgar.com/search",
        "https://futurerojgar.com/recent-jobs",
        "https://futurerojgar.com/classified-jobs"
    ]
    ...
    for test_url in test_urls:
        try:
            ...
            if response.status_code == 200:
                ...
                job_cards = soup.select('.job-card, .job-item, .job-listing, .job')
                if job_cards:
                    print(f"📋 Found {len(job_cards)} potential job cards")
                
                if job_count >= 2 or 'job' in title.lower() or job_cards:
                    working_urls.append((test_url, title))
                    print(f"✅ Looks like a job-related page!")
                    
                    # Save this page for analysis
                    filename = test_url.replace('https://futurerojgar.com', '').replace('/', '_') or '_main'
                    with open(f'test/futurerojgar{filename}.html', 'w', encoding='utf-8') as f:
                        f.write(soup.prettify())
                    print(f"💾 Saved to test/futurerojgar{filename}.html")
...
def analyze_job_patterns(url):
    """Analyze job listing patterns on FutureRojgar."""
    ...
    try:
        ...
        job_selectors = [
            ...
            '.post', '.entry', '.item', '.recent-job', '.classified-job'
        ]
        ...
        # Look for job statistics/counts
        print("\n📊 Looking for job counts:")
        stats_patterns = [
...
def test_job_detail_page(job_urls):
    ...
    try:
        ...
        with open('test/futurerojgar_job_detail.json', 'w', encoding='utf-8') as f:
            json.dump(job_data, f, indent=2)
        print(f"\n💾 Saved job detail analysis to test/futurerojgar_job_detail.json")
...
def main():
    print("🧪 Analyzing FutureRojgar Website Structure")
    print("🎯 Target: 30k+ jobs claimed by the website\n")
    
    # Step 1: Find working URLs
    working_urls = analyze_futurerojgar_structure()
    
    if not working_urls:
        print("\n❌ No working URLs found for FutureRojgar")
    ...
    print("\n" + "="*60)
    print("📊 FUTUREROJGAR ANALYSIS SUMMARY")
    print("="*60)
    ...
    print("\n🎯 Next Steps:")
    print("1. Update config/settings.py with FutureRojgar selectors")
    print("2. Create FutureRojgar scraper script")
    print("3. Test with limited pages")
    print("4. Scale to collect thousands of jobs")
    print("="*60)
